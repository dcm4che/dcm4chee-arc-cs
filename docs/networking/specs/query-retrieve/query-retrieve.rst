...
DCM4CHEE archive is configured so that the DCM4CHEE SCP AE and STORAGE-SCU AE share the same Application Entity Title.
...
Note that the STORAGE-SCU AE and DCM4CHEE SCP AE use the same Implementation Class UID. All DCM4CHEE archive AEs use the same Implementation Version Name. This Version Name is updated with each new release of the product software, as the different AE versions are never released independently.
...
1. Peer AE requests retrieval of Study, Series, or Images from DCM4CHEE SCP AE (C-MOVE-RQ).
2. DCM4CHEE SCP AE signals STORAGE-SCU AE to send the image Composite SOP Instances indicated in the C-MOVE-RQ to the C-MOVE Destination AE.
...
DCM4CHEE SCP Application Entity Specification
"""""""""""""""""""""""""""""""""""""""""""""""""""
...
The DCM4CHEE SCP AE provides Standard Conformance to the following DICOM V3.0 SOP Classes:

.. csv-table:: Table 4.2.2.2.1-1.: SOP Classes for DCM4CHEE SCP AE
   :header: "SOP Class Name", "SOP Class UID", "SCU", "SCP"
...
.. csv-table:: Table 4.2.2.2.2-1.: DICOM Application Context for DCM4CHEE SCP AE
   :file: common/storage-query-retrieve-workflow-general.csv
...
.. csv-table:: Table 4.2.2.2.2-2.: Number of Associations as a SCP for DCM4CHEE SCP AE
   :file: storage-scu-query-retrieve-scp-number-of-associations.csv
...
The DCM4CHEE SCP AE does not support asynchronous communication (multiple outstanding transactions over a single Association). All Association requests must be completed and acknowledged before a new operation can be initiated.

.. csv-table:: Table 4.2.2.2.2-3.: Asynchronous Nature as a SCP for DCM4CHEE SCP AE
...
.. csv-table:: Table 4.2.2.2.2-4.: DICOM Implementation Class and Version for DCM4CHEE SCP AE
   :file: common/storage-query-retrieve-implementation-identifying-information.csv
...
The DCM4CHEE SCP AE does not initiate Associations.
...
The DCM4CHEE SCP AE accepts Associations only if they have valid Presentation Contexts. If none of the requested Presentation Contexts are accepted then the Association Request itself is rejected. It can be configured to only accept Associations with certain hosts (using TCP/IP address) and/or Application Entity Titles.
If DCM4CHEE SCP AE receives a query (C-FIND) request then the response(s) will be sent over the same Association used to send the C-FIND-Request.
If DCM4CHEE SCP AE receives a retrieval (C-MOVE) request then the responses will be sent over the same Association used to send the C-MOVE-Request. The DCM4CHEE SCP AE will notify the STORAGE-SCU to send the requested SOP Instances to the C-MOVE Destination. The STORAGE-SCU AE notifies the DCM4CHEE SCP AE of the success or failure of each attempt to send a Composite SOP Instance to the peer C-MOVE Destination AE. The DCM4CHEE SCP AE then sends a C-MOVE Response indicating this status after each attempt. Once the STORAGE-SCU AE has finished attempting to transfer all the requested SOP Instances, the DCM4CHEE SCP AE sends a final C-MOVE Response indicating the overall status of the attempted retrieval.
...
1. Peer AE opens an Association with the DCM4CHEE SCP AE.
2. Peer AE sends a C-FIND-RQ Message
3. DCM4CHEE SCP AE returns a C-FIND-RSP Message to the peer AE with matching information. A C-FIND-RSP is sent for each entity matching the identifier specified in the C-FIND-RQ. A final C-FIND-RSP is sent indicating that the matching is complete.
...
3. DCM4CHEE SCP AE notifies the STORAGE-SCU AE to send the Composite SOP Instances to the peer C-MOVE Destination AE as indicated in the C-MOVE-RQ.
4. After attempting to send a SOP Instance, the STORAGE-SCU AE indicates to the DCM4CHEE SCP AE whether the transfer succeeded or failed. The DCM4CHEE SCP AE then returns a C-MOVE-RSP indicating this success or failure.
5. Once the STORAGE-SCU AE has completed all attempts to transfer the SOP Instances to the C-MOVE Destination AE, or the first failure occurred, the DCM4CHEE SCP AE sends a final C-MOVE-RSP indicating the overall success or failure of the retrieval.
...
The DCM4CHEE SCP AE may reject Association attempts as shown in the table below. The Result, Source and Reason/Diag columns represent the values returned in the corresponding fields of an ASSOCIATE-RJ PDU. The following abbreviations are used in the Source column:
...
.. csv-table:: Table 4.2.2.2.4-1.: Accepted Presentation Contexts By the DCM4CHEE SCP AE
...
DCM4CHEE SCP AE will accept Presentation Contexts as shown in the following table:

.. csv-table:: Table 4.2.2.2.4-2.: Accepted Presentation Contexts By the DCM4CHEE SCP AE
   :header: "Abstract Syntax Name", "Abstract Syntax UID", "Transfer Syntax Name", "Transfer Syntax UID", "Role", "Extended Negotiation"
...
.. csv-table:: Table 4.2.2.2.4-5.: DCM4CHEE SCP AE C-FIND Response Status Return Behavior
   :header: "Service Status", "Further Meaning", "Error Code", "Behaviour"
...
.. csv-table:: Table 4.2.2.2.4-7.: DCM4CHEE SCP AE Communication Failure Behavior
   :header: "Exception", "Behaviour"
